function clscentral_account_edit(){
	this.Fn_Initial = function(){
		$(document).ready(function(){
			...
			$('#cbeditnoexpire').unbind("change");
			$('#cbeditnoexpire').change(function(){	
				
				if($("#cbeditnoexpire").is(':checked')) {	
			...
			// 	// console.log('click_btn-add-ok');
				if (CheckData()){
					...
					$.post ('/config/central_account_edit_update',{ 
						id : id, 
...
////////////////////////////////////////////////////////////////////////////////////////////////////////
		...
		function find(){
			$.post('/config/central_account_edit_find',{id : id}).success(function(result){				
				if (result.length > 0) {
					// console.log(result[0].passPolicy);
					$('#central-edit-name-surname').val(result[0].FullName);
					$('#central-edit-username').val(result[0].userName);
					$('#central-edit-role').val(result[0].roleLevel);
					$('#central-edit-enable').val(result[0].userState);
						if (result[0].startDate !== '' && result[0].startDate !== undefined) {
		            		var datest = formatDate(result[0].startDate)}	                    	
		            	else {datest = ''}            		
					$('#central-edit-startdate').val(datest);
						if (result[0].expireDate !== '' && result[0].expireDate !== undefined) {
		            		var dateexp = formatDate(result[0].expireDate)}	                    	
		            	else {dateexp = ''}  
					$('#central-edit-expiredate').val(dateexp);
					$('#central-edit-branchfilter').val(result[0].branchFilter);

						if ($('#central-edit-expiredate').val() == '') {						
							$('#cbeditnoexpire').prop('checked', true);
							$("#central-edit-expiredate").val("");
							$("#central-edit-expiredate").prop('disabled', true);
						} else if ($('#central-edit-expiredate').val() !== ''){
							$("#cbeditnoexpire").prop('checked', false);
							$("#central-edit-expiredate").prop('disabled', false);
						};

						if (result[0].passPolicy == false) {
		...
		function role(){
			$.post('/config/central_account_edit_role',{'data' : ''}).success(function(role){
...
//			     } 
			...
			// 		return false;
			// }
			if($.trim($('#central-edit-expiredate').val()) == '' && ($("#cbnoexpire").is(':not(:checked)'))) {
